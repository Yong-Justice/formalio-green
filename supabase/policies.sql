...
create policy "users can update own draft reports" on public.reports for update to authenticated using (auth.uid() = user_id and status in ('reported', 'under_review')) with check (auth.uid() = user_id);
...
create policy "users can join missions as themselves" on public.mission_participants for insert to authenticated with check (auth.uid() = user_id);
create policy "users can update own participation" on public.mission_participants for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "users can read own proofs" on public.proofs for select to authenticated using (auth.uid() = user_id);
...
create policy "users can read own points" on public.eco_points for select to authenticated using (auth.uid() = user_id);
