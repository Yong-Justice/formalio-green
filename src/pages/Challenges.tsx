import { Bell, CheckCircle2, Clock3, Leaf, Recycle, Sprout, Trash2, Users } from 'lucide-react';
    ...
    <main className="flex flex-1 flex-col overflow-y-auto bg-white px-4 pb-7 pt-6">
      ...
      <section className="mt-4 space-y-3">
        {visibleChallenges.length ? (
          visibleChallenges.slice(0, 3).map((challenge, index) => <ChallengeCard key={challenge.id} challenge={challenge} featured={index === 0} />)
        ) : (
          <div className="rounded-2xl border border-slate-100 p-8 text-center text-slate-500">No completed challenges yet.</div>
        )}
      </section>

      <section className="mt-4 overflow-hidden rounded-2xl bg-green-dark p-4 text-white shadow-[0_10px_24px_rgba(6,78,59,0.2)]">
        <div className="flex items-center gap-4">
          <div className="flex h-20 w-24 items-end justify-center rounded-xl bg-primary/20">
            <Sprout size={58} className="text-lime-300" />
          </div>
          <div>
            <h2 className="text-xl font-extrabold">Small actions, big impact.</h2>
            <p className="mt-1 text-base leading-snug text-white/90">Challenge yourself. Inspire others. Build a <span className="font-extrabold text-lime-300">greener Cameroon.</span></p>
          </div>
        </div>
      </section>
    </main>
